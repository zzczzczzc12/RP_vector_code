...
def create_personality_vector(positive_list, negative_list, dataset_num=2, method='combine_vector_control',step=2):
    assert method in ['combine_dataset', 'combine_vector','combine_vector_control']
    ...
    # another posibility? step=1
        ...
        return control_vector
    # another posibility? step=i    
    elif method == 'combine_vector_control':
        ctrl_vectors = []
        for i in range(0, len(positive_list), step):
            pos_adj = positive_list[i:i+step]
            neg_adj = negative_list[i:i+step]
            result = make_control_vector(
                [pos_adj], [neg_adj], dataset_num=dataset_num)
            vec = result.forward()
            ctrl_vectors.append(vec)
...
if __name__ == '__main__':
    ...
    level_personality = [random.uniform(-1.5, 1.5) for _ in range(50)]
    settings = {
